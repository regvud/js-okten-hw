...
for (const course of coursesAndDurationArray) {
    ...
    h3.innerText = `${course.title} ${course.monthDuration}`
...
for (const course of coursesAndDurationArray) {
    ...
    paragraph.innerText = `${course.monthDuration}`;

    containerItem.append(h1, paragraph)
...
for (const simpson of simpsons) {
    ...
    h5.innerHTML = `<i>${simpson.info}</i>`

    let img = document.createElement('img');
    img.src = simpson.photo;

    container.append(h3, h5, img)
    document.body.appendChild(container)
}
...
for (const course of coursesArray) {
    ...
    for (const module of course.modules) {
        let li = document.createElement('li');
        li.innerHTML = `<u>${module}</u>`;
        ul.appendChild(li)
    }

    container.append(titleBlock, durationBlock, modulesBlock)
    document.body.appendChild(container)
}
